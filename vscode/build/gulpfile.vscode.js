...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	return () => {
		...
		if (platform === 'win32') {
			...
			// Rename Electron binary to Lean4Code
			const appRoot = path.join(BUILD_ROOT, 'electron', 'Lean4Code.app', 'Contents', 'MacOS');
			const oldBin = path.join(appRoot, 'Electron');
			const newBin = path.join(appRoot, 'Lean4Code');
