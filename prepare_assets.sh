...
if [[ "${OS_NAME}" == "osx" ]]; then
  if [[ "${CI_BUILD}" != "no" ]]; then
    ...
    if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
      codesign --deep --force --verbose --timestamp=none --sign "${CERTIFICATE_OSX_ID}" "${APP_NAME} - Insiders.app"
    else
      codesign --deep --force --verbose --timestamp=none --sign "${CERTIFICATE_OSX_ID}" "${APP_NAME}.app"
    fi
