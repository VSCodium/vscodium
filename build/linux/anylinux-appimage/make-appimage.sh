...
QUALITY=$(jq -r '.quality' ./AppDir/bin/resources/app/product.json)
URLPROTOCOL=$(jq -r '.urlProtocol' ./AppDir/bin/resources/app/product.json)
VERSION=$(jq -r '.version' ./AppDir/bin/resources/app/product.json)
...
export ICON="src/${QUALITY}/resources/linux/code.svg"
export DEPLOY_GTK=1
