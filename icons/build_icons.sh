...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    rsvg-convert -w 655 -h 655 "icons/lean4code/lean4code.svg" -o "code_logo.png"
...
build_darwin_types() { # {{{
  rsvg-convert -w 128 -h 128 "icons/lean4code/lean4code.svg" -o "code_logo.png"
...
build_linux_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png" ]]; then
    rsvg-convert -w 512 -h 512 "icons/lean4code/lean4code.svg" -o "${SRC_PREFIX}src/${QUALITY}/resources/linux/code.png"
  fi
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    cp "icons/lean4code/lean4code.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
...
build_server() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/server/favicon.ico" ]]; then
    rsvg-convert -w 256 -h 256 "icons/lean4code/lean4code.svg" -o "code_logo.png"
    convert "code_logo.png" -define icon:auto-resize=256,128,96,64,48,32,24,20,16 "${SRC_PREFIX}src/${QUALITY}/resources/server/favicon.ico"
    rm "code_logo.png"
  fi

  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/server/code-192.png" ]]; then
    rsvg-convert -w 192 -h 192 "icons/lean4code/lean4code.svg" -o "${SRC_PREFIX}src/${QUALITY}/resources/server/code-192.png"
  fi

  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/server/code-512.png" ]]; then
    rsvg-convert -w 512 -h 512 "icons/lean4code/lean4code.svg" -o "${SRC_PREFIX}src/${QUALITY}/resources/server/code-512.png"
  fi
} # }}}
...
build_windows_type() { # {{{
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "icons/lean4code/lean4code.svg" -o "code_logo.png"
...
build_windows_types() { # {{{
  ...
  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "icons/lean4code/lean4code.svg" -o "code_logo.png"
