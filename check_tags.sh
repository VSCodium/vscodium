...
if [ "$GITHUB_TOKEN" != "" ]; then
  if [ "$VSCODIUM_ASSETS" != "null" ]; then
    if [[ "$TRAVIS_OS_NAME" == "osx" ]]; then
      ...
      if [[ "$HAVE_32_RPM" != "true" ]]; then
        echo "Building on Linux x32 because we have no RPM"
        export SHOULD_BUILD="yes"
      fi
      if [[ "$HAVE_32_DEB" != "true" ]]; then
        echo "Building on Linux x32 because we have no DEB"
        export SHOULD_BUILD="yes"
      fi
      if [[ "$HAVE_32_TAR" != "true" ]]; then
        echo "Building on Linux x32 because we have no TAR"
        export SHOULD_BUILD="yes"
      fi
      if [[ "$SHOULD_BUILD" != "yes" ]]; then
        echo "Already have all the Linux x32 builds"
      fi
    else
