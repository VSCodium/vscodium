...
# Check for custom release version from patch rebuild
if [[ -n "${CUSTOM_RELEASE_VERSION}" ]]; then
  echo "🔥 Using custom RELEASE_VERSION from patch rebuild: ${CUSTOM_RELEASE_VERSION}"
...
if [[ -z "${RELEASE_VERSION}" ]]; then
  ...
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    ...
    if [[ "${RELEASE_VERSION}" =~ ^([0-9]+\.[0-9]+\.[0-5])[0-9]+$ ]]; then
      MS_TAG="${BASH_REMATCH[1]}"
    else
      echo "Error: Bad RELEASE_VERSION: ${RELEASE_VERSION}"
      exit 1
